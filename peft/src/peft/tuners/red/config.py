...
@dataclass
class REDConfig(PeftConfig):
    """
    This is the configuration class to store the configuration of a [`REDModel`].
    ...
    Args:
        ...
        feedforward_modules (`Optional[Union[List[str], str]]`):
            ...
            have RED vectors multiplied to the input, instead of the output. `feedforward_modules` must be a name or
            a subset of names present in `target_modules`.
        fan_in_fan_out (`bool`):
            Set this to True if the layer to replace stores weight like (fan_in, fan_out). For example, gpt-2 uses
            `Conv1D` which stores weights like (fan_in, fan_out) and hence this should be set to `True`.
        modules_to_save (`Optional[List[str]]`):
            List of modules apart from RED layers to be set as trainable and saved in the final checkpoint.
        init_red_weights (`bool`):
            Whether to initialize the vectors in the RED layers, defaults to `True`. Setting this to `False` is
            discouraged.
        layer_type (`str`):
            设置编辑向量的类型
    """
    ...
    target_modules: Optional[Union[List[str], str]] = field(
        ...
        metadata={
            "help": (
                "List of module names or regex expression of the module names to replace with RED."
    ...
    modules_to_save: Optional[List[str]] = field(
        default=None,
        metadata={
            "help": "List of modules apart from RED layers to be set as trainable and saved in the final checkpoint. "
            "For example, in Sequence Classification or Token Classification tasks, "
            "the final layer `classifier/score` are randomly initialized and as such need to be trainable and saved."
        },
    )
    init_red_weights: bool = field(
        default=True,
        metadata={"help": "Whether to initialize the vectors in the RED layers."},
    )

    layer_type: str = field(
        default="all",
        metadata={"help": "The type of vectors in the RED layers."},
    )

    def __post_init__(self):
        self.peft_type = PeftType.RED
        self.target_modules = (
